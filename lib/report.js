...
class Report {
    constructor(options)
    {
        ...
        const labels = {};
        const entity_types = [];
        (options.entries ?? options.documents).forEach(entry => { // options.documents is deprecated
            let candidates = [];
            (entry.candidates ?? entry.annotations).forEach(anno => { // entry.annotations is deprecated
                ++this.documentCount;
                const user = anno.user ?? anno.label ?? null; // anno.label is deprecated
                delete anno.user;
                delete anno.label;
            ...
            this.entries.push({
                docid: entry.docid ?? entry.id, // entry.id is deprecated
    ...
    display(root)
    {
        ...
        this.entries.forEach((entry, i) => {
            ...
            entry.candidates.forEach((can, j) => {
                const cls = can.user === null ?
                    "brat" :
                    ( can.user == "*" || can.user == "accepted" ? // user "accepted" is deprecated
                          "brat accept" :
                          "brat reject" );
                ...
                if (can.user !== null) {
                    const label = can.user !== "*" ? can.user : "accepted";
                    Report.appendElement(div, "span", label, {class: "label"});
                }
                index[entry.docid][can.user ?? "*"] = id;
            });
        });
