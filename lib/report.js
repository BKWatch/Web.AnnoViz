...
class Report {
    constructor(title, subtitle, date, style, width, schema, entries)
    {
        this.title = title;
        ...
        entries.forEach(item => {
            let candidates = [];
            (item.candidates ?? item.annotations).forEach(anno => {
            ...
            this.entries.push({id: item.id, candidates: candidates});
        });
        if (!this.schema) {
    ...
    display(root)
    {
        ...
        const index = {};
        this.entries.forEach((doc, i) => {
            const row = Report.appendElement(table, "tr");
            Report.appendElement(row, "td", doc.id);
            const td = Report.appendElement(row, "td");
            index[doc.id] = {};
            doc.candidates.forEach((can, j) => {
                const cls = can.class !== null ?
                    "brat " + can.class:
                    "brat";
                ...
                index[doc.id][can.label ?? "accepted"] = id;
            });
        });
