...
class Ani {
    ...
    {
        let pgn = options.pagination ?? param('pagination');
        ...
        if (typeof(pgn) == "boolean") {
            pgn = pgn ? ANI_DEFAULT_PAGINATION : null;
        } else if (typeof(pgn) != "object") {
            throw new Error(`Invalid pagination: expected string, boolean, or object; found ${typeof(pgn)}`);
        }
        if (pgn != null && options.entries === undefined) {
            throw new Error(`Pagination not supported for ${this.entries}`);
        }
        this.pagination = pgn;
    }
