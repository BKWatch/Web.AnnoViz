        ...
        $.getScript(SCHEMA_DIR + this.schema + ".js", function() {
            ...
            that.documents.forEach(function(doc, i) {
                ...
                doc.annotations.forEach(function(anno, j) {
                    ...
                    let cls = anno.class !== null ? ' ' + anno.class + '"' : '';
                    let label = anno.label !== null ?
            ...
            setTimeout(function() {
                for (const docId in layout) {
                    for (const label in layout[docId]) {
                        let rect = document.getElementById(layout[docId][label]).parentNode.getBoundingClientRect();
