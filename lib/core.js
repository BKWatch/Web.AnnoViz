...
function Report(title, subtitle, date, style, schema, documents, width) {
    this.title = title;
...
Report.load = function(name, width, success) {
    ...
    $.getScript(SCHEMA_DIR + this.schema + ".js", function() {
        ...
        that.documents.forEach(function(doc, i) {
            ...
            doc.annotations.forEach(function(anno, j) {
                ...
                var style = that.width != null ?
                    'style="width:' + that.width + 'px;display:inline-block" ' :
                    '';
