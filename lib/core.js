...
Report.load = function(name, success) {
    $.getScript(DATA_DIR + name + ".js").done(function() {
        if (success) {
            success(new Report(
                data.title,
                data.subtitle,
        ...
        $("#title").html("Report Not Found");
        $("#report").html("No such report:<i>" + name + "</i>");
    });
};
...
Report.prototype.display = function() {
    ...
    $.getJSON(SCHEMA_DIR + this.schema + ".json").done(function(schema) {
        ...
        that.documents.forEach(function(doc, i) {
            ...
            doc.annotations.forEach(function(anno, j) {
                ...
                layout[doc.id][anno.label || "accepted"] = id;
            });
        ...
        $("#report").append(table);
        that.embed(schema, layout);


    }).fail(function() {
        $("#title").html("Schema Not Found");
        $("#report").html("No such schema:<i>" + that.schema + "</i>");
    });
};
...
Report.prototype.embed = function(schema, layout) {
    var count = 0;
    var that = this;
    this.documents.forEach(function(doc, i) {
        doc.annotations.forEach(function(anno, j) {
            ...
            dispatcher.on('doneRendering', function() {
                ++count;
                if (count == that.annotationCount && param('dump') !== null) {
                    that.report(layout);
                }
            });
            Util.embed(that.makeId(i, j), schema, newData, [], dispatcher);
        });
    });
};

Report.prototype.report = function(layout) {
    for (docId in layout) {
        for (label in layout[docId]) {
            var rect = document.getElementById(layout[docId][label]).childNodes[0].getBoundingClientRect();
